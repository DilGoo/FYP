...
add2log(f"Batch Size is {BATCH_SIZE}.\n")
...
fullDataset = brats3dDataset(dataPath, downscale=False)
...
try:
    for epoch in range(NUM_EPOCHS):
        ...
        optimizer.zero_grad()       
        for i, (inputs, target) in enumerate(batchloop):
            inputs = inputs.to(device)               # Use cuda if available
            target = target.to(device)
            
            pred = model(inputs)
            loss = criterion(pred, target)           # one graph is created here
            loss.backward()                          # graph is cleared here
            if (i+1) % BATCH_SIZE == 0:
                optimizer.step()
                batchloop.set_description(f"Epoch number {epoch+1}, Loss: {loss.item()}")
                trainlosses.append(loss.item())
                trainscores.append(score_fun(pred, target))
                optimizer.zero_grad()
